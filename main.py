...
import asyncio
import json
from TTS import xhTTs
from langchain.chains.llm import LLMChain
from langchain_core.prompts import PromptTemplate
from langchain_core.runnables import Runnable
from CustomPrompt import prompt_temp
import DataClear
...
def test():
    ...
    IChroma.chroma_save_embedding(xinghuo_response)
    result = IChroma.chroma_query_embedding("赛亚人", 2)
    print("向量查询", result)
    return result
...
if __name__ == '__main__':
    ...
    # # 创建一个提示模板
    #
    prompt = PromptTemplate(input_variables=["input_text"], template=prompt_temp.prompt_template)
    # # 创建新的 Runnable 实例
    chain = XinghuoRunnable(prompt_template=prompt_temp.prompt_template, llm=xinhuo_llm)
    # 学习外部知识
    user_input= Loader.html_loader(Loader.url)


    #使用 Xinghuo API 获取响应
    xinghuo_response = chain.invoke({"input_text": user_input})
    json.loads(xinghuo_response.strip().replace("```", "").replace("json", ""))
    # print(xinghuo_response)
    # xinghuo_response = DataClear.jsondata
    print(xinghuo_response)
    role = xinghuo_response['主角姓名']


    IChroma.chroma_save_embedding(xinghuo_response)
    question='你好啊，悟空'
    result = IChroma.chroma_query_embedding(question, 3)
    print(result)
    prompt1 = PromptTemplate(input_variables=["book","role", "tips", "question"], template=prompt_temp.prompt_question)
    ...
    print("星火答案：" + xinghuo_response)
    # 调用TTS服务转换成语音
    # 运行 WebSocket 连接
    asyncio.run(xhTTs.tts_websocket(xinghuo_response))
